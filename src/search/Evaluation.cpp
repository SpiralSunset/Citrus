...
constexpr int Evaluation::piece_value(Board::PieceType pt) {
	switch (pt) {
...
constexpr int Evaluation::phase_weight(Board::PieceType pt) {
	switch (pt) {
		case Board::PieceType::Pawn:
			return 0;
		case Board::PieceType::Knight:
...
int Evaluation::evaluate(Position &pos) {
	...
	while (occ) {
		int sq = pop_lsb(occ);
		Board::Piece piece = pos.get_board().get_square(sq);
		
		int value = piece_value(piece.pt);
		if (piece.c == Board::Color::cWhite) {
